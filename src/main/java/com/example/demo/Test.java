...
public class Test {
    ...
    public void test() throws Exception {
        int concurrency = DemoApplication.concurrency;
        int repeat = DemoApplication.repeat;
        if (DemoApplication.testUpdate) {
            updateTest(concurrency, repeat);
        } else {
...
//        }
    ...
    private List<Long> userIds = new ArrayList<>(100000);
    private List<Long> orderIds = new ArrayList<>(100000);
    private List<Long> txnIds = new ArrayList<>(100000);
    private List<Long> paymentIds = new ArrayList<>(100000);

    private void updateTest(int concurrency, int repeat) throws Exception {
        long start = System.currentTimeMillis();
...
//
            for (int i = 0; i < 1000; i++) {
                userIds.add(i, uidGenerator.getUID());
                orderIds.add(i, uidGenerator.getUID());
                txnIds.add(i, uidGenerator.getUID());
                paymentIds.add(i, uidGenerator.getUID());

                inPstmt.setLong(1, txnIds.get(i)); // txn_id
                inPstmt.setLong(2, userIds.get(i)); //user_id
                inPstmt.setString(3, "txn_type");
                inPstmt.setString(4, "txn_state");
                inPstmt.setLong(5, 100); // txn_order_amount
                inPstmt.setString(6, "txn_order_currency");
                ...
                inPstmt.setInt(13, 0); // disabled
                inPstmt.setInt(14, 1); // version
                inPstmt.setLong(15, orderIds.get(i)); // order_id
                inPstmt.setString(16, "started"); // order_state
                inPstmt.setString(17, "code1"); // order_error_code
                inPstmt.setString(18, "type1"); // order_type
                inPstmt.setLong(19, 4); // order_version
                inPstmt.setString(20, "{}");// order_items
                inPstmt.setLong(21, paymentIds.get(i)); // payment_id
                inPstmt.setLong(22, 5); // payment_version
                inPstmt.setString(23, "paid");// payment_state
                inPstmt.setString(24, "c1");// payment_error_code
                inPstmt.setString(25, "{}");// comments
                    ...
                    try {
                        for (int i = 0; i < repeat; i++) {
                            ...
                            try (Connection conn = dataSource.getConnection()) {
                                ...
                                conn.setAutoCommit(false);
                                int index = random.nextInt(userIds.size());
                                setUpdateParam(updateStmt, txnIds.get(index), userIds.get(index),
                                    orderIds.get(index), paymentIds.get(index));
                                updateStmt.execute();
                                updateStmt.clearParameters();
                                conn.commit();
                            }
